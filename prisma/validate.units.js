...
const allowedTextKeys = new Set(['ally_targeting', 'damage_type', 'stack_duration', 'targeting', 'amp_curve']);
...
function validateStatsObject(stats, path = 'ability.stats') {
  ...
  for (const [k, v] of Object.entries(stats)) {
    ...
    if (k.startsWith('dmg_')) {
      if (!isDamageKeyOK(k)) errors.push(`${path}.${k}: invalid damage key; use dmg_{ad|ap|magic|true|mixed}_{flat|ratio_pct|hp_pct}`);
      if (!(isNum(v) || isNumArr3(v))) errors.push(`${path}.${k}: value must be number or [n1,n2,n3]`);
      continue;
    }
    ...
    if (k === 'tactician_power') {
      if (!Array.isArray(v)) { errors.push(`${path}.${k}: must be an array`); continue; }
      v.forEach((entry, i) => {
        if (typeof entry !== 'object' || entry == null) { errors.push(`${path}.${k}[${i}]: must be an object`); return; }
        if (!isNum(entry.min_level)) errors.push(`${path}.${k}[${i}].min_level must be a number`);
        if (entry.effects == null || typeof entry.effects !== 'object') { errors.push(`${path}.${k}[${i}].effects must be an object`); return; }
...
(async () => {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (!parsed.success) {
      issues.push(`${where}: ${parsed.error.issues.map((x) => `${x.path.join('.')}: ${x.message}`).join('; ')}`);
      continue;
    }
    ...
    if (!dbUnitNames.has(entry.name)) warns.push(`${where}: unit not in DB roster (name mismatch?)`);
  ...
  if (missingInJson.length) warns.push(`Units in DB but missing from JSON (${missingInJson.length}): ${missingInJson.join(', ')}`);
