...
async function seedTiers() {
  ...
  for (const [traitName, tiers] of Object.entries(data)) {
    const trait = await prisma2.trait.findUnique({ where: { setId_name: { setId: set.id, name: traitName } } });
    if (!trait) { console.warn(`[warn] missing trait in DB: ${traitName}`); continue; }
...
seedTiers()
  ...
  .finally(async () => { await prisma2.$disconnect(); });